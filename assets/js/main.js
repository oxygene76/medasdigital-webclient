...
// MedasDigital WebClient Integration
// ===================================

console.log('🚀 Starting MedasDigital WebClient via Vite...');

// ===================================
// CosmJS ES Module Import (funktioniert mit Vite!)
// ===================================

try {
    console.log('📚 Loading CosmJS via Vite ES modules...');
    
    // Import CosmJS - Das funktioniert jetzt mit Vite!
    const { SigningStargateClient, coins } = await import('@cosmjs/stargate');
    
    // Global verfügbar machen für bestehenden Code
    window.SigningStargateClient = SigningStargateClient;
    window.coins = coins;
    window.cosmjsReady = true;
    
    console.log('✅ CosmJS loaded via Vite middleware!');
    console.log('SigningStargateClient type:', typeof SigningStargateClient);
    console.log('coins function type:', typeof coins);
    
    // Event dispatchen für bestehenden Code
    window.dispatchEvent(new Event('cosmjs-loaded'));
    
} catch (error) {
    console.error('❌ CosmJS loading failed:', error);
    
    // Fallback für bestehenden Code
    window.SigningStargateClient = null;
    window.coins = () => [];
    window.cosmjsReady = false;
    window.dispatchEvent(new Event('cosmjs-failed'));
}

// ===================================
// Ihre bestehenden Scripts importieren OHNE Initialisierung
// (Terminal wird aus bestehender main.js initialisiert)
// ===================================

console.log('📚 Loading application scripts...');

try {
    // Lade alle Ihre bestehenden Scripts
    await import('../assets/js/config.js');
    console.log('✅ Loaded: config.js');
    
    await import('../assets/js/mock-data.js');
    console.log('✅ Loaded: mock-data.js');
    
    await import('../assets/js/keplr-manager.js');
    console.log('✅ Loaded: keplr-manager.js');
    
    await import('../assets/js/staking-manager.js');
    console.log('✅ Loaded: staking-manager.js');
    
    await import('../assets/js/ui-manager.js');
    console.log('✅ Loaded: ui-manager.js');
    
    await import('../assets/js/staking-helpers.js');
    console.log('✅ Loaded: staking-helpers.js');
    
    // WICHTIG: main.js zuletzt laden (enthält Terminal-Initialisierung)
    await import('../assets/js/main.js');
    console.log('✅ Loaded: main.js (Terminal wird dort initialisiert)');
    
    await import('../assets/js/mobile-navigation.js');
    console.log('✅ Loaded: mobile-navigation.js');
    
    console.log('🎉 All application scripts loaded successfully!');
    
} catch (error) {
    console.error('❌ Failed to load application script:', error);
    console.log('⚠️ Continuing with available scripts...');
}
...
window.debugCosmJS = function() {
    console.log('🔍 CosmJS Debug Information:');
    console.log('=========================');
    console.log('Environment: Vite + Express');
    console.log('window.cosmjsReady:', window.cosmjsReady);
    console.log('window.SigningStargateClient:', window.SigningStargateClient ? 'Available' : 'Not found');
    console.log('window.coins:', window.coins ? 'Available' : 'Not found');
    console.log('window.terminal:', window.terminal ? 'Initialized' : 'Not found');
    console.log('import.meta.env.DEV:', import.meta.env.DEV);
    console.log('import.meta.env.PROD:', import.meta.env.PROD);
    
    return {
        cosmjsReady: window.cosmjsReady,
        hasSigningClient: !!window.SigningStargateClient,
        hasCoins: !!window.coins,
        hasTerminal: !!window.terminal,
        viteEnv: import.meta.env,
        mode: 'vite-express-hybrid'
    };
};
...
console.log('🚀 MedasDigital WebClient ready via Vite!');
console.log('💡 CosmJS available via ES modules');
console.log('🔥 Hot Module Reload active');
console.log('🎮 Terminal will be initialized from main.js');
